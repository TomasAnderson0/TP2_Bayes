...
ggplot(muestras_kuma) + geom_histogram(aes(x = obs, fill = cadena), color = "#4b4b4b", bins = 24 ) + facet_wrap(~ cadena, labeller = labeller(cadena = label_facet(muestras_kuma$cadena, "Concentración"))) + tema +
  ...
  scale_fill_discrete(guide = "none",labels = c("1", "5", "10"), name = c("Concentración"), type = c("lightblue2", "violet", "lightgreen"))
```
...
geom_hline(yintercept =0.05, linetype = "dashed", color = "blue") + geom_hline(yintercept = -0.05, linetype = "dashed", color = "blue") +
  scale_color_discrete(guide = "none",labels = c("1", "5", "10"), name = c("Concentración"), type = c("lightblue2", "violet", "lightgreen")) + tema
```
...
```{r, eval=FALSE}
# Se utiliza como dstribución de salto propuesto una normal bivariada
...
Se controlará que tan buenas son las muestras obtenidas a traves de la evaluación de la cadena generada por el método y de qué tanto se asimilan las probabilidades estimadas por la distribución muestral obtenida a las reales. La normal bivariada de interés viene dada por los siguientes parámetros.
...
\end{array}
$$
Usando la función de Metropolis-Hastings se obtiene un muestra de 5000 observaciones. Se utiliza la matriz de varianzas y covarianzas 
...
muestra_normal = sample_mh_2d(5000, p_inicial = c(1,1), d_objetivo = d_objetivo_normal, matrix_var = matrix(c(1, .3, .3, 1), nrow = 2))
...
$$
En estadística bayesiana, la densidad de la distribución a *posteriori* suele tomar formas semejantes a dicha función y, más específicamente, con la que se trabaja en este informe es la función $p^*$:
$$
p^*(x_1, x_2 \mid a, b) = \exp \left\{-\left[(a - x_1) ^ 2 + b(x_2 - x_1^2) ^ 2\right] \right\}
$$

Se utiliza la función con los valores $0.5$ para los parámetros a y b.
