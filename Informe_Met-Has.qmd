...
\hline
 & \multicolumn{3}{c|}{Concentración} \\
 & 1 & 5 & 10  \\
\hline
  Tasa de aceptación & `r round(tasa_kuma_1,3)` & `r round(tasa_kuma_1,2)` & `r round(tasa_kuma_3,3)` \\
\hline
\end{tabular}
\caption{Table to test captions and labels.}
\label{table:1}
...
ggplot(acf_kuma) + geom_line(aes(x = Rezago, y = ACF)) + geom_point(aes(x = Rezago, y = ACF)) + facet_wrap(~ Concentracion) + geom_hline(yintercept = 0) + geom_vline(xintercept = 0)+
